...
const ALIASES_DISTRIBUICAO = [
  ...
  { chave: "quantidade de reunioes", palavras: ["quantidade", "reuni"] },
];
...
async function getClosersElegiveis() {
  ...
  // Índice por nome normalizado -> linha da aba distribuicao_reuniao.
  // Filtra só linhas de Closer: a aba vai ganhar linhas de SDR no futuro
  // (pra outra automação), e não devem entrar aqui.
  const indiceDist = {};
  ...
  for (const row of rowsColab) {
    ...
    if (!linhaDist) continue; // sem linha na distribuicao_reuniao -> fora, por enquanto

    const reunioesHoje = parseFloat(getCampo(linhaDist, mapaDist, COLUNA_CONTADOR) || "0") || 0;
