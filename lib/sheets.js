 ...
 *
 * Precisa ser async: a lib google-spreadsheet só preenche sheet.headerValues
 * depois de loadHeaderRow() (ou getRows(), que chama isso por baixo). Chamar
 * antes disso causa "Header values are not yet loaded".
 */
async function montarMapaCabecalho(sheet) {
  // sheet.headerValues é um getter que lança erro se acessado antes do load —
  // por isso nunca checamos "if (!sheet.headerValues)" antes: sempre chamamos
  // loadHeaderRow() incondicionalmente, e só então lemos headerValues.
  ...
  // Aliases tolerantes a pequenos erros de digitação no cabeçalho real da
  // planilha (ex: "Euqipe" em vez de "Equipe") — a normalização por acento/
  // espaço não resolve isso, então aqui procuramos por PALAVRAS-CHAVE dentro
  // do cabeçalho em vez de exigir o texto inteiro idêntico.
  const aliases = [
