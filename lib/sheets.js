...
/**
 * Desconta 1 do contador do dia (RECEBIDAS_HOJE) de um closer pelo Nome —
 * usado quando um deal distribuído automaticamente é reatribuído manualmente
 * pra outra pessoa, corrigindo a contagem de quem "perdeu" a reunião.
 * Nunca deixa o contador ficar negativo.
 */
async function descontarContadorPorNome(nomeCloser) {
  const doc = await getDoc();
  ...
  const nomeNorm = norm(nomeCloser);
  const row = rows.find((r) => norm(getCampo(r, mapa, COLUNA_NOME_DISTRIBUICAO)) === nomeNorm);
  if (!row) {
    throw new Error(`Closer '${nomeCloser}' não encontrado em ${ABA_DISTRIBUICAO} pra descontar o contador.`);
  }

  const atual = parseFloat(row.get(headerRecebidas) || "0") || 0;
  const novoValor = Math.max(0, atual - 1);
  row.set(headerRecebidas, novoValor);
  await row.save();
  return novoValor;
}
 ...
 * (a mais recente = a última que bate, já que addRow sempre acrescenta no
 * final). Retorna null se o deal nunca passou pela distribuição automática.
 */
...
/** Marca ALTERADO = "Sim" na linha de log — evita descontar o contador duas vezes pro mesmo deal. */
async function marcarLogAlterado(logEntry) {
  const headerReal = logEntry.mapa[norm(COLUNA_LOG_ALTERADO)];
  if (!headerReal) {
    throw new Error(`Coluna '${COLUNA_LOG_ALTERADO}' não encontrada no cabeçalho de ${ABA_LOG}.`);
  }
  logEntry.row.set(headerReal, "Sim");
